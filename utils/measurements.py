...
def clear_mot_hungarian(stDB, gtDB, threshold):
    ...
    for i in range(stDB.shape[0]):
        # Convert the frame number in stDB to an integer
        frame_number = int(stDB[i, 0])  # Convert to integer

        # Check if frame_number is in gt_frames_list
        if frame_number in gt_frames_list:
            frame = gt_frames_list.index(frame_number)

            # Extract the ID from stDB and ensure it's the correct data type
            stDB_id = stDB[i, 1]
            if isinstance(stDB_id, float):
                stDB_id = int(stDB_id)  # Convert to integer if it's a float

            # Check if the ID exists in st_ids
            if stDB_id in st_ids:
                sid = np.where(st_ids == stDB_id)[0][0]
                st_inds[frame][sid] = i
            else:
                print(f"ID {stDB_id} not found in st_ids")
