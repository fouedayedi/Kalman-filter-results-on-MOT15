...
def read_txt_to_struct(fname):
    ...
    with open(fname, 'r') as fid:
        for line in fid:
            ...
            # Convert frame number to integer and other elements to float
            line_elements[0] = int(line_elements[0])  # Frame number as integer
            line_elements[1:] = map(float, line_elements[1:])  # Other values as float
            data.append(line_elements)

    data = np.array(data)
    
    # Change point-size format to two-points format
    # Ensure bbox coordinates are processed correctly
    if data.shape[1] >= 6:  # Check if bbox columns exist
        data[:, 4:6] += data[:, 2:4]
